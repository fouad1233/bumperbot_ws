...
SimpleController::SimpleController(const std::string& name)
                                  : Node(name)
                                  , left_wheel__prev_pos_(0.0)
                                  , right_wheel__prev_pos_(0.0)
                                  , x_(0.0)
                                  , y_(0.0)
                                  , theta_(0.0)
{
...
{
    double dp_left = msg.position.at(1) - left_wheel__prev_pos_;
    double dp_right = msg.position.at(0) - right_wheel__prev_pos_;

    rclcpp::Time msg_time = msg.header.stamp;
    rclcpp::Duration dt = (msg_time - prev_time_);

    left_wheel__prev_pos_ = msg.position.at(1);
    right_wheel__prev_pos_ = msg.position.at(0);
    ...
    double linear = wheel_radius_ * (fi_right + fi_left) / 2.0;
    double angular = wheel_radius_ * (fi_right - fi_left) / wheel_separation_;
    RCLCPP_INFO_STREAM(get_logger(), "Linear vel: " << linear << " Angular vel: " << angular);
    ...
    x_ += d_s * cos(theta_);
    y_ += d_s * sin(theta_);
    RCLCPP_INFO_STREAM(get_logger(), "Robot Position -> x: " << x_ << " m, y: " << y_ << " m, theta: " << theta_ << " rad");
